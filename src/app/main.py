...
def get_data(database, search_term):
    ...
    print(
        "Total number of documents returned for the above search query: "
        + str(len(uids))
    )
...
def preprocess_summary_data():
    # Extract the species
    st.session_state["df_summary"]["Species"] = st.session_state["df_summary"][
        "Title"
    ...
    st.session_state["df_summary"] = st.session_state["df_summary"].reindex(
        columns=column_order
    )

st.title("Neglected Diagnostics: Perform Genetic Testing At Scale!")

def get_download_ids(df: pd.DataFrame) -> List[int]:
    return df["Id"].unique()
...
with st.form("query"):
    ...
    if submitted:
        ...
        print(
            "Summary form submission count",
            st.session_state["summary_form_submit_count"],
        )

        st.session_state["df_summary"] = get_data(database, search_term)
        print(
            "Number of rows in summary after fetching data: ",
            len(st.session_state["df_summary"]),
        )
        preprocess_summary_data()

        st.session_state["SUMMARY_FORM_SUBMIT"] = True

df_aggrid_output = pd.DataFrame()
if st.session_state["SUMMARY_FORM_SUBMIT"]:
    ...
    options_builder.configure_column(
        "Title", filter="agMultiColumnFilter", filterParams={"maxNumConditions": 4}
    )
    ...
    df_aggrid_output = grid_table["data"]
    # st.write('Filtered rows')
    # st.dataframe(filtered_df)

df_filter_output = components.filter_dataframe(df_aggrid_output)
if st.session_state["FILTER"]:
    st.dataframe(df_filter_output)
    ...
    if st.button("Prepare Download"):
        ...
        if len(file_buffer.getvalue()) == 0:
            st.write("No data to download. Please refine your filters")
        else:
            st.write("Fetched data successfully!")
            st.download_button(
                label="Download", data=file_buffer.getvalue(), file_name="data.fasta"
            )
        # st.write(document)
